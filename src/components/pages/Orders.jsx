...
import { Search, Filter, QrCode, PlusCircle } from "lucide-react";
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {orders.map((order, idx) => (
                  <tr
                    key={order.id}
                    className={`${idx % 2 === 0 ? "bg-white/5" : "bg-transparent"} border-t border-white/10 hover:bg-white/10 transition`}
                  >
            ...
            <div className="bg-neutral-900 rounded-2xl border border-white/10 p-4 sm:p-6 relative w-full max-w-md sm:max-w-lg shadow-xl">
              ...
              <div className="overflow-hidden rounded-xl border border-white/10">
                <BarcodeScannerComponent
                  width={"100%"}
                  height={250}
                  onUpdate={(err, result) => { if (result) setScannedCode(result.text); }}
                />
              </div>
              {scannedCode && (
                <div className="mt-4 p-3 sm:p-4 bg-emerald-500/20 border border-emerald-500/40 rounded-xl text-sm sm:text-base text-center flex flex-col items-center gap-1 break-all">
                  ...
                  <span className="font-mono text-white bg-neutral-900 px-2 py-1 rounded text-sm sm:text-base break-all">{scannedCode}</span>
                </div>
              )}
            </div>
          </div>
